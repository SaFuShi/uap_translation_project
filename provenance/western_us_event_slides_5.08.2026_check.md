# プロベナンスチェック: ai_reading_003_western_us_event_slides_note_version.md
...
| 項目 | 値 |
|---|---|
| ドラフト | `note_drafts/ai_reading_003_western_us_event_slides_note_version.md` |
...
- **記事での出現:** `45メートル`
- **コンテキスト:** `...トで物体を再探索した際、「ある地点でビームが遠くまで届かず約45メートル先で止まったが、その後は再び遠くまで届いた」との証言も記録さ...`
- **PDFで検索:** `45` → **見つかりません**
...
|---|---|---|---|
| `18` | altitude | p.2 | `bject to have been between 12-18 meters in diameter. artist ` |
| `600` | altitude | p.2 | `istance of “approximately 500-600 meters,”* where it was per` |
| `1050` | altitude | p.2 | `sess the object to have been ~1050 meters away from the obse` |
| `100` | altitude | p.3 | `ce.” the object stopped about 100 meters off the side of the` |
| `6` | altitude | p.1 | `nts each (usper1 through usper6) independently describe seei` |
| `2` | duration | p.2 | `object to have been between 12-18 meters in diameter. artist` |
| `30` | duration | p.4 | `f day: pre-dawn hours • about 30 minutes after the “dark kit` |
| `USPER1` | proper_noun | p.1 | `orcement special agents each (usper1 through usper6) indepen` |
| `AARO` | proper_noun | p.2 | `easurements later gathered by aaro assess the object to have` |
| `USPER5` | proper_noun | p.2 | `w enforcement special agents (usper5 and usper6) witness a g` |
