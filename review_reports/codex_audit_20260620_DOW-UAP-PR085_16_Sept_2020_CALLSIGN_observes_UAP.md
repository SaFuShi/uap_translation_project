...
P1-2-META PASS duration 284.17秒、1280×720、H.264、30fps、音声ありは thumbnails metadata / ffprobe と整合する。
...
IMG-OBSERVATION WARN タイトル・要点で「IR」と断定調の表現がある一方、本文では「IRセンサーと推定されるが確認できない」としており、IR種別の事実/推定の分離が不十分。
...
W-07: 画像記事表現 | 「均一グレーIR映像」「グレースケールIR映像」 | 「グレースケール映像。IRセンサー映像の可能性はあるが、本文では断定しない」などに統一する。
W-08: 代表フレーム挿入行 | → 使用ファイル：thumbnails/DOW-UAP-PR085_16_Sept_2020_CALLSIGN_CALLSIGN_observes_UAP/frame_0000.png（note転記後にこの行を削除） | 公開用本文から作業用行を削除する。
---WARN_DETAILS_END---
---CODEX_AUDIT_END---