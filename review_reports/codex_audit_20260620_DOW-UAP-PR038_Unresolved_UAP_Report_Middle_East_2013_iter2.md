...
P1-DVIDS-TITLE BLOCK metadata/uap-csv-cache.csv の DVIDS Video Title は「Resolved as an Aircraft, Middle East 2013」だが、本文は「未解決」「物体の正体・種別は確認できず」としており、一次メタデータ上の重大な不一致が未説明。
P1-URL PASS 動画記事として WAR.GOV 公開ページと DVIDS URL が併記され、DVIDS ID 1006088 と整合する。
P1-TECH WARN ffprobe の format bit_rate は約2,308 kbps、video stream bit_rate は約2,204 kbpsであり、本文の「コンテナ」欄にあるビットレート表記はラベル上やや不正確。
P1-OFFICIAL-BLURB WARN metadata/uap-csv-cache.csv の公式説明にある USCENTCOM、AARO、IRセンサー、米軍プラットフォーム、映像内の動きの説明が本文で十分に反映・区分されていない。
...
---ITEMS_END---
---WARN_DETAILS_START---
W-01: 映像メタデータ | ビットレート：2,204 kbps | format全体なら約2,308 kbps、映像ストリームなら約2,204 kbpsとラベルを分ける。
W-02: 公式説明反映 | 映像（IRセンサーと推定されるが確認できない） | 公式説明由来の情報として「IRセンサー」「米軍プラットフォーム」「USCENTCOM」「AARO」を視覚観察とは別枠で記載する。
W-03: 記事構成 | 注意点および明示的な免責セクションなし | AI読解後に「注意点」、出典後に「免責」を独立セクションとして置く。
W-04: タイトル | # 【概要版#TBD】DoW DOW-UAP-PR038... | article_id 確定後に #TBD を除去した公開用タイトルへ確定する。
