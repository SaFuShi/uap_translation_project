...
P1-2 PASS ffprobe/metadata.jsonの動画仕様（286.37秒、1280x720、30fps、H.264、AAC、147.91MB）は本文と整合する。
P1-3 PASS CENTCOM、AOR、DVIDS、CALLSIGNは初出付近で補足されている。
P1-5 WARN source_registry.csv未登録のため、article_id・status・重複チェックの正式管理が未完了。
P2-1 WARN note転記後に削除すべき作業メモ行が本文内に残っている。
P2-2 PASS FMV/ISR等の主要な未説明略語は本文主要部に出ておらず、専門語補足の不足は重大ではない。
...
IMG-1 PASS 視覚観察（クロスヘア状表示、シアンマーカー、黒塗り、地形）と解釈は分離されている。
