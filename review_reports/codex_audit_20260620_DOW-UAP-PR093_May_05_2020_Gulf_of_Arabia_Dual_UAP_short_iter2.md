...
P1-1 WARN review_logs/source_registry.csv に対象PR093の登録がなく、記事ID・登録上の一次資料整合性が未確定。
P1-2 PASS 再生時間・解像度・フレームレート・音声有無はローカルffprobe結果およびthumbnail metadataと整合。
P1-3 PASS DVIDS、CENTCOM、AORなど主要略語には最低限の日本語補足あり。
P1-5 PASS PR093とPR095はMD5が異なる別ファイルであることを既存監査記録と記事内記述で確認。
...
P2-5 PASS note転記上の禁止フォーマットや作業用注釈の混入は検出なし。
...
W-04: 画像記事 | 本記事は約30秒のグレースケールIR映像クリップを扱います。 | 「グレースケール映像（IRセンサー由来の可能性はあるが、このフレームだけでは確認できない）」にする。
W-05: 関連ファイル | PR093（30.1秒）とPR095（288.5秒）はファイル名が同一ですが、MD5・再生時間が異なる別ファイルです。 | 「PR番号を除く記述部分が同一の別ファイル」など、実ファイル名との差分を明確化する。
---WARN_DETAILS_END---
---CODEX_AUDIT_END---