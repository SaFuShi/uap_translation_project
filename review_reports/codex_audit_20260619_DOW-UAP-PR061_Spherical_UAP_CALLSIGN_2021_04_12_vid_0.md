...
P1-2 PASS ffprobe/metadata.jsonの動画仕様（286.37秒、1280x720、30fps、H.264、AAC、約147.91MB）は本文と整合する。
...
IMG-1 PASS 視覚観察（クロスヘア状表示、シアンマーカー、黒塗り、地形）と解釈は概ね分離されている。
IMG-2 WARN 「追尾対象物」「追尾クロスヘア」は、表示機能未確認とする本文方針に対して追尾機能をやや強く示唆する。
SRC-1 PASS PR060/PR062/PR063との関連は「推定」「可能性」として留保され、同日関連ファイルの記述もfiles_catalog.csvと整合する。
---ITEMS_END---
---WARN_DETAILS_START---
...
W-02: note投稿本文 | → 使用ファイル：thumbnails/DOW-UAP-PR061_Spherical_UAP_CALLSIGN_2021_04_12_vid_0/frame_0060.png（note転記後にこの行を削除） | note本文には残さず、画像差し込み作業用メモとして削除する。
W-03: 画像記事の解釈分離 | 抽出した10フレームにおいて、追尾クロスヘアが指し示す中央付近に何らかの対象が存在する可能性はありますが、背景地形との区別が困難で確認できませんでした。 | 「クロスヘア状表示の中央付近に何らかの対象が存在する可能性はありますが、背景地形との区別が困難で確認できませんでした。」にする。
---WARN_DETAILS_END---
---CODEX_AUDIT_END---