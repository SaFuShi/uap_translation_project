...
P1-META-DATE WARN metadata/uap-csv-cache.csv のDVIDSタイトルには「13 Feb 23」があるが、本文は「月・曜日は不明」「2023年13日」としており、利用可能なメタデータ反映が不足している。
...
M1-DYNAMIC PASS 「移動」「追跡」「消失」等の動的事象を確定事実として断定していない。
M6-PROVENANCE-CAVEAT WARN uap-csv-cache.csv にあるAAROの説明、NORTHCOM AOR、chain-of-custody不足の注意書きが本文に反映されていない。
---ITEMS_END---
---WARN_DETAILS_START---
W-01: 文書メタデータ | Incident Date：2023年・13日（ファイル名「S0301_23」の「23」および「on_13_」より。files_catalog.csv では「2023」として記録。月・曜日は不明） | DVIDSタイトル由来情報として「2023年2月13日（DVIDSタイトルより。ただしfiles_catalog.csvのIncident Date欄は2023）」のように根拠を分ける。
W-02: AI読解 | **AI解析メモ：** 動画ファイル。ffprobeによる技術情報取得済み。 | AI解析メモは出典後または免責直前の所定位置に移す。
W-03: 代表フレーム直前 | → 使用ファイル：thumbnails/.../frame_0010.png（note転記後にこの行を削除） | 制作メモ行は本文から除き、必要ならキャプションだけを残す。
W-04: 出典・注意点 | 本文全体でAARO説明文のchain-of-custody注意とNORTHCOM AORが未反映 | 「AARO説明では、当該映像は2023年にNORTHCOM担当区域で運用された米軍プラットフォーム上の赤外線センサー由来の可能性があるとされる一方、資料群にはchain-of-custodyが十分でないものが含まれる」と出典由来の注意として短く補う。
---WARN_DETAILS_END---
---CODEX_AUDIT_END---