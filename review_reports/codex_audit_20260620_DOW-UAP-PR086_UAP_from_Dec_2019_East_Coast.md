...
P1-1 PASS ファイル名・DVIDS ID・Release Date・動画技術メタデータは files_catalog.csv および ffprobe結果と整合する。
...
P2-1B WARN note転記用の作業指示行が本文内に残っている。
...
W-02: 画像差し込み指示 | 「→ 使用ファイル：thumbnails/...（note転記後にこの行を削除）」 | note本文から作業指示を削除し、必要なら画像キャプションのみ残す。
W-03: 客観表現 | 「2019年12月に米国東海岸で観測されたUAPの映像」 | 「2019年12月に米国東海岸で記録されたとされるUAP映像」などにする。
W-04: 比較評価 | 「Release 02のDOW-UAP映像の中で最も視覚的に鮮明な事案の一つ」 | 根拠を示すか、「視覚的に比較的確認しやすい映像」に弱める。
