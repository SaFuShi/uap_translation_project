...
W-03: note投稿互換 | → 使用ファイル：thumbnails/...（note転記後にこの行を削除） | 公開本文から作業用行を削除する。
W-04: 画像記事 | グレースケールIR映像 | 「グレースケール映像（IRセンサー由来の可能性はあるが、このフレームだけでは確認できない）」にする。
W-05: 関連ファイル | PR093（30.1秒）とPR095（288.5秒）はファイル名が同一ですが、MD5（PR093: 5bf213a5b1a4d65c5c595b440ea07e19、PR095: 05cfb5e906e859814ae8f35f4df1a053）と再生時間が異なる別ファイルです。 | 「PR番号を除く記述部分が同一の別ファイル」など、実ファイル名との差分を明確化する。
---WARN_DETAILS_END---
---CODEX_AUDIT_END---